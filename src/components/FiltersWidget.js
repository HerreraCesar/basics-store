...
const FiltersWidget = () => {
    ...
    return (
        <>
        <div className={active === true? 'filters activated' : 'filters'} onClick={()=> {setActive(!active)}}>
            <h3>FILTROS</h3>
        </div>
        <div className={active === true? 'filtersBox activated' : 'filtersBox'}>
            <span>Ordenar por </span>
            <select className='select' onChange={(e) => {applyFilter(e.target.value)}}>
                <option value='priceUp' defaultValue>precio: de menor a mayor</option>
